...
uint8_t TS_IO_Read(uint8_t addr, uint8_t reg)
{
  ...
  HAL_I2C_Mem_Read(&hi2c1, addr, (uint16_t)reg, I2C_MEMADD_SIZE_8BIT, &read_value, 1, 1000);
  return read_value;
}
...
void Touch_Init(void)
{
	Touch_Reset();
	HAL_Delay(200);
//	touchID = Touch_ReadID(TS_I2C_ADDRESS);
...
uint8_t TS_GetState(TS_StateTypeDef *TS_State)
{
  ...
  if(TS_State->touchDetected)
  {
    for(index=0; index < TS_State->touchDetected; index++)
    {
		...
		TS_State->touchX[index] = x[index]*495/272; // correction touchscreen bug of my display
		TS_State->touchY[index] = y[index]*272/480; // correction touchscreen bug of my display

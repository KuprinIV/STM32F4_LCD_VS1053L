...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x00,
  0x02,                                 /* bInCollection */
  AUDIO_OUT_IF,                                 /* baInterfaceNr1 = Number of OUT Interface (Speaker AS) */
  AUDIO_IN_IF,									/* baInterfaceNr2 = Number of IN Interface (Mic AS) */
  /* 10 byte*/
  ...
  0x01,                                 /* bTerminalID */
  0x01,                                 /* wTerminalType 0x0101 = Microphone */
  0x01,
  0x00,                                 /* bAssocTerminal */
  0x01,                                 /* bNrChannels */
  ...
  AUDIO_INPUT_TERMINAL_DESC_SIZE, /* bLength = AUDIO_INPUT_TERMINAL_DESC_SIZE = 12 bytes*/
  AUDIO_INTERFACE_DESCRIPTOR_TYPE, /* bDescriptorType = AUDIO_INTERFACE_DESCRIPTOR_TYPE = 0x24 */
  AUDIO_CONTROL_INPUT_TERMINAL, /* bDescriptorSubtype = AUDIO_CONTROL_INPUT_TERMINAL = 0x02 */
  0x04, /* bTerminalID = ID4 */
  0x01, /* wTerminalType = 0x0201 = Microphone */
  0x02,
  0x00, /* bAssocTerminal = No Associated Terminal*/
  0x01, /* bNrChannels = 0x01 = Mono */
  ...
  AUDIO_INTERFACE_DESCRIPTOR_TYPE, /* bDescriptorType = AUDIO_INTERFACE_DESCRIPTOR_TYPE = 0x24 */
  AUDIO_CONTROL_FEATURE_UNIT, /* bDescriptorSubtype = AUDIO_CONTROL_FEATURE_UNIT = 0x06 */
  0x05, /* bUnitID = ID5 */
  ...
  0x09, /* bLength = 09 bytes */
  AUDIO_INTERFACE_DESCRIPTOR_TYPE, /* bDescriptorType = AUDIO_INTERFACE_DESCRIPTOR_TYPE = 0x24 */
  AUDIO_CONTROL_OUTPUT_TERMINAL, /* bDescriptorSubtype = AUDIO_CONTROL_OUTPUT_TERMINAL = 0x03 */
  0x06, /* bUnitID = ID6 */
  ...
  AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
  USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
  AUDIO_IN_EP, //AUDIO_IN_EP            /* bEndpointAddress 1 in endpoint*/
  USBD_EP_TYPE_ISOC,                    /* bmAttributes */
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  (void)USBD_LL_PrepareReceive(pdev, AUDIO_OUT_EP, haudio->out_packet_buffer,
                               AUDIO_OUT_PACKET);

//  /* Make first mic transmission */
//  USBD_LL_FlushEP  (pdev, AUDIO_IN_EP);
//  USBD_LL_Transmit (pdev, AUDIO_IN_EP, haudio->in_packet_buffer, AUDIO_IN_PACKET);
    ...
    case USB_REQ_TYPE_STANDARD:
      switch (req->bRequest)
      {
        ...
        case USB_REQ_GET_INTERFACE:
          if (pdev->dev_state == USBD_STATE_CONFIGURED)
          {
            (void)USBD_CtlSendData(pdev, (uint8_t *)&haudio->alt_settings[req->wIndex], 1U);
          }
          else
          {
            USBD_CtlError(pdev, req);
...
/**
  ...
  * @retval pointer to descriptor out_packet_buffer
  */
static uint8_t *USBD_AUDIO_GetCfgDesc(uint16_t *length)
{
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == AUDIO_OUT_EP)
  {
    ...
    ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData)->PeriodicTC(haudio->out_packet_buffer,
                                                           PacketSize, AUDIO_OUT_TC);

    /* Prepare Out endpoint to receive next audio packet */
    (void)USBD_LL_PrepareReceive(pdev, AUDIO_OUT_EP,
                                 haudio->out_packet_buffer,
                                 AUDIO_OUT_PACKET);
  }
